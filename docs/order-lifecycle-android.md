...
| `item.created` / `item.updated`          | Reload the menu via `loadMenu()` — even when `currentOrderId` is null (type-select / browsing still needs the picker to reflect POS admin `isActive` toggles). `order.item.*` starts with `order.` so it never matches.                                                                                                   |
| reconnect (`realtimeClient.reconnected`) | Reload the menu via `loadMenu()` so item events missed during a WS drop do not leave a stale catalog. Order refetch on reconnect is out of scope.                                                                                                                                                                         |
