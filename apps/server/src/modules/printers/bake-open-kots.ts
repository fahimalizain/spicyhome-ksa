 ...
 *
 * Pure module (no NestJS) so it can be driven three ways:
 *   1. `bazel run //apps/server:bake_open_kots -- --db <path> --out <path>`
 *      (slice 3 wires the js_binary; this module already carries the CLI main)
 *   2. `node scripts/printing/kitchen/bake-open-kots.mjs` (slice 3 wrapper)
 *   3. jest: `bazel test //apps/server:test --test_filter=bake-open-kots`
 */
 ...
 * Do not edit by hand — regenerate with the bake-open-kots wrapper script.
 *
...
export function bakeOpenKotsCli(argv: string[]): number {
  ...
  try {
    ...
    const outPath = outFlag
      ...
      : join(root, 'scripts', 'printing', 'kitchen', 'out', 'send-open-kots.js');
