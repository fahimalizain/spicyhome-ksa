...
describe('schema — migrations', () => {
    let migrationsDir;
    beforeAll(() => {
        migrationsDir = (0, migrate_1.findMigrationsDir)();
    });
    it('applies migrations cleanly to a :memory: database', () => {
        const sqlite = new better_sqlite3_1.default(':memory:');
        expect(() => (0, migrate_1.applyMigrations)(sqlite, migrationsDir)).not.toThrow();
        sqlite.close();
    });
    it('applies migrations cleanly to a temp file database', () => {
        const sqlite = (0, migrate_1.createTestDb)(migrationsDir);
        expect(() => sqlite.close()).not.toThrow();
    });
    describe('journal idempotency', () => {
        it('__drizzle_migrations has exactly 5 rows after apply', () => {
            const sqlite = new better_sqlite3_1.default(':memory:');
            (0, migrate_1.applyMigrations)(sqlite, migrationsDir);
            const rows = sqlite.prepare('SELECT COUNT(*) as cnt FROM __drizzle_migrations').get();
        ...
        it('second applyMigrations is a no-op on the same DB', () => {
            const sqlite = new better_sqlite3_1.default(':memory:');
            (0, migrate_1.applyMigrations)(sqlite, migrationsDir);
            const before = sqlite.prepare('SELECT COUNT(*) as cnt FROM __drizzle_migrations').get().cnt;
            // Second apply must not throw and must not add entries
            expect(() => (0, migrate_1.applyMigrations)(sqlite, migrationsDir)).not.toThrow();
            const after = sqlite.prepare('SELECT COUNT(*) as cnt FROM __drizzle_migrations').get().cnt;
...
describe('schema — invariants', () => {
    let sqlite;
    beforeAll(() => {
        const migrationsDir = (0, migrate_1.findMigrationsDir)();
        sqlite = (0, migrate_1.createTestDb)(migrationsDir);
    ...
    describe('foreign keys', () => {
      ...
      `);
            const roleId = sqlite.prepare('SELECT last_insert_rowid() as id').get().id;
      ...
      `);
            // Verify
            const user = sqlite.prepare('SELECT * FROM users WHERE username = ?').get('testuser');
      ...
      `);
            const userId = sqlite.prepare('SELECT last_insert_rowid() as id').get().id;
            sqlite.exec(`
      ...
      `);
            const doId = sqlite.prepare('SELECT last_insert_rowid() as id').get().id;
            sqlite.exec(`
      ...
      `);
        ...
        it('order_items.order_id references orders.id with cascade delete', () => {
            const now = Math.floor(Date.now() / 1000);
            // Set up dependencies
            const userRow = sqlite.prepare('SELECT id FROM users LIMIT 1').get();
            sqlite.exec(`
        INSERT INTO day_openings (business_date, status, opened_at, opened_by, created_at, updated_at)
      ...
      `);
            const orderId = sqlite.prepare('SELECT last_insert_rowid() as id').get().id;
            sqlite.exec(`
    ...
    describe('unique constraints', () => {
        it('users.username is unique', () => {
            const now = Math.floor(Date.now() / 1000);
            const roleId = sqlite.prepare('SELECT id FROM user_roles LIMIT 1').get().id;
            sqlite.exec(`
        INSERT INTO users (username, pin_hash, name, role_id, created_at, updated_at)
      ...
      `);
        ...
        it('orders.uuid is unique', () => {
            const now = Math.floor(Date.now() / 1000);
            const doId = sqlite.prepare('SELECT id FROM day_openings LIMIT 1').get().id;
            sqlite.exec(`
        INSERT INTO orders (order_no, uuid, type, day_opening_id, status, created_at, updated_at)
      ...
      `);
            const refundId = sqlite.prepare('SELECT last_insert_rowid() as id').get().id;
            sqlite.exec(`
      ...
      `);
            const orderId3 = sqlite.prepare('SELECT last_insert_rowid() as id').get().id;
            sqlite.exec(`
        INSERT INTO order_refunds (order_id, user_id, method_id, method_title, subtotal_halalas, vat_halalas, total_halalas, reason, created_at)
        VALUES (${orderId3}, ${userId}, 'cash', 'Cash', 1000, 150, 1150, 'test', ${now})
      `);
            const refundId3 = sqlite.prepare('SELECT last_insert_rowid() as id').get().id;
            expect(() => sqlite.exec(`
          INSERT INTO zatca_credit_notes (order_id, refund_id, related_invoice_uuid, icv, uuid, invoice_hash, prev_invoice_hash, xml, qr_tlv, status, total_halalas, vat_halalas, reason, created_at, updated_at)
      ...
      `);
            const orderId2 = sqlite.prepare('SELECT last_insert_rowid() as id').get().id;
            sqlite.exec(`
        INSERT INTO order_refunds (order_id, user_id, method_id, method_title, subtotal_halalas, vat_halalas, total_halalas, reason, created_at)
        VALUES (${orderId2}, ${userId}, 'cash', 'Cash', 1000, 150, 1150, 'test', ${now})
      `);
            const refundId2 = sqlite.prepare('SELECT last_insert_rowid() as id').get().id;
            expect(() => sqlite.exec(`
          INSERT INTO zatca_credit_notes (order_id, refund_id, related_invoice_uuid, icv, uuid, invoice_hash, prev_invoice_hash, xml, qr_tlv, status, total_halalas, vat_halalas, reason, created_at, updated_at)
      ...
      `);
        ...
        it('zatca_credit_notes.reported_at column is nullable, zatca_invoices.reported_at column exists', () => {
            ...
            const cnInfo = sqlite.prepare('PRAGMA table_info(zatca_credit_notes)').all();
            const cnReportedAt = cnInfo.find((c) => c.name === 'reported_at');
            expect(cnReportedAt).toBeDefined();
            expect(cnReportedAt.notnull).toBe(0); // nullable
            const invInfo = sqlite.prepare('PRAGMA table_info(zatca_invoices)').all();
            const invReportedAt = invInfo.find((c) => c.name === 'reported_at');
