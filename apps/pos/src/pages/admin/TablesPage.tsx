  ...
  return (
    <div className="h-full overflow-y-auto p-4">
      ...
      <form onSubmit={handleSave} className="bg-gray-800 rounded-xl p-4 mb-4 space-y-3">
        <h2 className="text-sm font-semibold text-gray-300">{editId ? 'Edit Table' : 'New Table'}</h2>
        <div>
      ...
      <div className="space-y-1">
        {tables.map((t) => (
          <div key={t.id} className="flex items-center justify-between bg-gray-800 rounded-lg px-3 py-2">
            ...
            <button onClick={() => editTable(t)} className="touch-target text-xs text-brand-400 hover:text-brand-300 px-2 py-1">Edit</button>
          </div>
        ))}
      </div>
    </div>
